...
bool flight_due() {
  ...
  return phase == P_IDLE && (first || millis() - lastCycle >= FL_INTERVAL);
}
...
static void parse(Stream &s) {
  JsonDocument filter;
  JsonObject fac = filter["aircraft"][0].to<JsonObject>();
  fac["flight"] = true;
  fac["alt_baro"] = true;
  fac["track"] = true;
  fac["dst"] = true;
  ...
  switch (phase) {
    case P_IDLE:
      if (first || millis() - lastCycle >= FL_INTERVAL) {
        ...
        if (!netsched_can_start(NS_FLIGHT)) return;   // not our turn in the cascade
        if (!tls_try_acquire()) { netsched_done(NS_FLIGHT); return; }  // safety; cascade prevents overlap
        first = false;
  ...
  if (c->connect(host.c_str(), FL_PORT)) {
    ...
    while (millis() - t0 < timeoutMs) {
      ...
      if (skip_proxy_headers(*c)) {
        while (millis() - t0 < timeoutMs) {
          ESP.wdtFeed();
          if (skip_chunk_prefix(*c)) { parse(*c); ok = gData.valid; break; }
