...
async function loadHydrologic(lat, lon) {
  ...
  let nearest = null;
  const ranks = rankBasins(
    lat,
    lon,
    dams || [],
    dated.map((d) => d.basin_name),
  );
  const top = ranks[0];
  if (top) {
    ...
    if (hit)
      nearest = {
        ...
        volume:
          typeof hit.current_volume === "string" && hit.current_volume !== ""
            ? Number(hit.current_volume)
            : null,
  ...
  const droughts = (dated.filter((d) => Number(d.state) < 5) || []).map((d) => ({
    ...
    color: d.color || null,
    volume:
      typeof d.current_volume === "string" && d.current_volume !== ""
        ? Number(d.current_volume)
        : null,
